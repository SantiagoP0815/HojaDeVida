  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      if (validPassword) {
        ...
        return done(null, user); // devolver el usuario, la ruta hará el redirect
      } else {
      ...
      return done(null, false, { message: 'El número de documento no está registrado.' });
    }
  } catch (err) {
...
passport.serializeUser((user, done) => {
  // Usar el identificador primario de la tabla personas
